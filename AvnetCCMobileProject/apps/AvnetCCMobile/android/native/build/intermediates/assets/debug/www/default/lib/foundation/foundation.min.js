...
/* JavaScript content from lib/foundation/foundation.min.js in folder common */
...
!function(){"use strict";function FoundationAnimation(){function toggleAnimation(element,futureState){futureState?element.addClass(activeGenericClass):element.removeClass(activeGenericClass)}function animate(element,futureState,animationIn,animationOut){function finishAnimation(){deregisterElement(element),reset(),element[0].style.transitionDuration="",element.removeClass(activation?"":activeGenericClass),reflow(),timedOut=!1}function cancelAnimation(element){deregisterElement(element),angular.element(element).off(events.join(" ")),timedOut=!1}function registerElement(el){var elObj={el:el,animation:self},inProgress=animations.filter(function(obj){return obj.el===el});if(inProgress.length>0){var target=inProgress[0].el[0];inProgress[0].animation.cancelAnimation(target)}animations.push(elObj)}function deregisterElement(el){{var index;animations.filter(function(obj,ind){obj.el===el&&(index=ind)})}index>=0&&animations.splice(index,1)}function reflow(){return element[0].offsetWidth}function reset(){element[0].style.transitionDuration=0,element.removeClass(initClasses.join(" ")+" "+activeClasses.join(" ")+" "+animationIn+" "+animationOut)}var timedOut=!0,self=this;self.cancelAnimation=cancelAnimation;var animationClass=futureState?animationIn:animationOut,activation=futureState,initClass=activation?initClasses[0]:initClasses[1],activeClass=activation?activeClasses[0]:activeClasses[1];registerElement(element),reset(),element.addClass(animationClass),element.addClass(initClass),element.addClass(activeGenericClass),reflow(),element[0].style.transitionDuration="",element.addClass(activeClass),element.one(events.join(" "),function(){finishAnimation()}),setTimeout(function(){timedOut&&finishAnimation()},3e3)}var animations=[],service={},initClasses=["ng-enter","ng-leave"],activeClasses=["ng-enter-active","ng-leave-active"],activeGenericClass="is-active",events=["webkitAnimationEnd","mozAnimationEnd","MSAnimationEnd","oanimationend","animationend","webkitTransitionEnd","otransitionend","transitionend"];return service.animate=animate,service.toggleAnimation=toggleAnimation,service}angular.module("foundation.core.animation",[]).service("FoundationAnimation",FoundationAnimation)}();
!function(){"use strict";function FoundationApi(FoundationAnimation){function subscribe(name,callback){return listeners[name]||(listeners[name]=[]),listeners[name].push(callback),!0}function publish(name,msg){listeners[name]||(listeners[name]=[]),listeners[name].forEach(function(cb){cb(msg)})}function getSettings(){return settings}function modifySettings(tree){return settings=angular.extend(settings,tree)}function generateUuid(){var uuid="";do{uuid+="zf-uuid-";for(var i=0;15>i;i++)uuid+=Math.floor(16*Math.random()).toString(16)}while(!uniqueIds.indexOf(uuid));return uniqueIds.push(uuid),uuid}function toggleAnimate(element,futureState){FoundationAnimation.toggleAnimate(element,futureState)}function closeActiveElements(options){var self=this;options=options||{};var activeElements=document.querySelectorAll(".is-active[zf-closable]");activeElements.length&&angular.forEach(activeElements,function(el){options.exclude!==el.id&&self.publish(el.id,"close")})}function animate(element,futureState,animationIn,animationOut){FoundationAnimation.animate(element,futureState,animationIn,animationOut)}var listeners=[],settings={},uniqueIds=[],service={};return service.subscribe=subscribe,service.publish=publish,service.getSettings=getSettings,service.modifySettings=modifySettings,service.generateUuid=generateUuid,service.toggleAnimate=toggleAnimate,service.closeActiveElements=closeActiveElements,service.animate=animate,service}function FoundationAdapter(foundationApi){function activate(target){foundationApi.publish(target,"show")}function deactivate(target){foundationApi.publish(target,"hide")}var service={};return service.activate=activate,service.deactivate=deactivate,service}function Utils(){function throttleUtil(func,delay){var timer=null;return function(){var context=this,args=arguments;null===timer&&(timer=setTimeout(function(){func.apply(context,args),timer=null},delay))}}var utils={};return utils.throttle=throttleUtil,utils}angular.module("foundation.core",["foundation.core.animation"]).service("FoundationApi",FoundationApi).service("FoundationAdapter",FoundationAdapter).factory("Utils",Utils),FoundationApi.$inject=["FoundationAnimation"],FoundationAdapter.$inject=["FoundationApi"]}();
!function(){"use strict";function uiAnimation($rootScope){function enterAnimation(element,done){var scope=element.scope();if(scope.vars&&scope.vars.animationIn){var animationIn=scope.vars.animationIn,animationOut=scope.vars.animationOut||"",initial="ng-enter",activate="ng-enter-active",timedOut=!0;element.parent().addClass(parentStyle),element.removeClass(activate+" "+initial+" "+animationIn+" "+animationOut),element[0].style.transitionDuration=0,element.addClass(animationIn),element.addClass(initial),$rootScope.$digest(),element[0].style.transitionDuration="",element.addClass(activate);var finishAnimation=function(){element.parent().removeClass(parentStyle),element.removeClass(activate+" "+initial+" "+animationIn+" "+animationOut),timedOut=!1,done()};element.one(events.join(" "),function(){finishAnimation()}),setTimeout(function(){timedOut&&finishAnimation()},3e3)}else done();return function(){}}function leaveAnimation(element,done){var scope=element.scope();if(scope.vars&&scope.vars.animationOut){var animationIn=scope.vars.animationIn||"",animationOut=scope.vars.animationOut,initial="ng-leave",activate="ng-leave-active",timedOut=!0;element.removeClass(activate+" "+initial+" "+animationIn+" "+animationOut),element[0].style.transitionDuration=0,element.addClass(animationOut),element.addClass(initial),$rootScope.$digest(),element[0].style.transitionDuration="",element.addClass(activate);var finishAnimation=function(){element.parent().removeClass(parentStyle),element.removeClass(activate+" "+initial+" "+animationIn+" "+animationOut),timedOut=!1,done()};element.one(events.join(" "),function(){finishAnimation()}),setTimeout(function(){timedOut&&finishAnimation()},3e3)}else done();return function(){}}var events=["webkitAnimationEnd","mozAnimationEnd","MSAnimationEnd","oanimationend","animationend","webkitTransitionEnd","otransitionend","transitionend"],parentStyle="position-absolute",animation={};return animation.enter=enterAnimation,animation.leave=leaveAnimation,animation}angular.module("foundation.dynamicRouting.animations",["ngAnimate","foundation.dynamicRouting"]).animation(".ui-animation",uiAnimation),uiAnimation.$inject=["$rootScope","$state"]}();
!function(){"use strict";function FoundationState($stateProvider){function getData(page){var data={vars:{}};return page.data&&("object"==typeof page.data.vars&&(data.vars=page.data.vars),delete page.data.vars,angular.extend(data,page.data)),delete page.data,angular.extend(data.vars,page),data}function buildState(path,state){return{templateUrl:path,controller:getController(state)}}function getController(state){var ctrl=state.controller||"DefaultController";return/\w\s+as\s+\w/.test(ctrl)||(ctrl+=" as PageCtrl"),ctrl}var complexViews={};this.registerDynamicRoutes=function(routes){var dynamicRoutes=routes||foundationRoutes;angular.forEach(dynamicRoutes,function(page){if(page.hasComposed)angular.isDefined(complexViews[page.parent])||(complexViews[page.parent]={children:{}}),page.controller&&(page.controller=getController(page)),complexViews[page.parent].children[page.name]=page;else if(page.composed)angular.isDefined(complexViews[page.name])||(complexViews[page.name]={children:{}}),page.controller&&(page.controller=getController(page)),angular.extend(complexViews[page.name],page);else{var state={url:page.url,templateUrl:page.path,parent:page.parent||"",controller:getController(page),data:getData(page)};$stateProvider.state(page.name,state)}}),angular.forEach(complexViews,function(page){var state={url:page.url,parent:page.parent||"",data:getData(page),views:{"":buildState(page.path,page)}};angular.forEach(page.children,function(sub){state.views[sub.name+"@"+page.name]=buildState(sub.path,page)}),$stateProvider.state(page.name,state)})},this.$get=angular.noop}function DefaultController($scope,$stateParams,$state){var params={};angular.forEach($stateParams,function(value,key){params[key]=value}),$scope.params=params,$scope.current=$state.current.name,$state.current.views?($scope.vars=$state.current.data.vars,$scope.composed=$state.current.data.vars.children):$scope.vars=$state.current.data.vars}function DynamicRoutingConfig(FoundationStateProvider){FoundationStateProvider.registerDynamicRoutes(foundationRoutes)}function DynamicRoutingRun($rootScope,$state,$stateParams){$rootScope.$state=$state,$rootScope.$stateParams=$stateParams}angular.module("foundation.dynamicRouting",["ui.router"]).provider("$FoundationState",FoundationState).controller("DefaultController",DefaultController).config(DynamicRoutingConfig).run(DynamicRoutingRun),FoundationState.$inject=["$stateProvider"],DefaultController.$inject=["$scope","$stateParams","$state"],DynamicRoutingConfig.$inject=["$FoundationStateProvider"],DynamicRoutingRun.$inject=["$rootScope","$state","$stateParams"]}();
!function(){"use strict";function mqInitRun(mqInit){mqInit.init()}function FoundationMQInit(helpers,foundationApi,u){function init(){var mediaQueries,extractedMedia;helpers.headerHelper(["foundation-mq"]),extractedMedia=helpers.getStyle(".foundation-mq","font-family"),mediaQueries=helpers.parseStyleToObject(extractedMedia);for(var key in mediaQueries)mediaQueries[key]="only screen and (min-width: "+mediaQueries[key].replace("rem","em")+")";foundationApi.modifySettings({mediaQueries:angular.extend(mediaQueries,namedQueries)}),window.addEventListener("resize",u.throttle(function(){foundationApi.publish("resize","window resized")},50))}var factory={},namedQueries={"default":"only screen",landscape:"only screen and (orientation: landscape)",portrait:"only screen and (orientation: portrait)",retina:"only screen and (-webkit-min-device-pixel-ratio: 2),only screen and (min--moz-device-pixel-ratio: 2),only screen and (-o-min-device-pixel-ratio: 2/1),only screen and (min-device-pixel-ratio: 2),only screen and (min-resolution: 192dpi),only screen and (min-resolution: 2dppx)"};return factory.init=init,factory}function mqHelpers(){function headerHelper(classArray){for(var i=classArray.length,head=angular.element(document.querySelectorAll("head"));i--;)head.append('<meta class="'+classArray[i]+'" />')}function getStyle(selector){var elem=document.querySelectorAll(selector)[0],style=window.getComputedStyle(elem,null);return style.getPropertyValue("font-family")}function parseStyleToObject(str){var styleObject={};return"string"!=typeof str?styleObject:(str=str.trim().slice(1,-1))?styleObject=str.split("&").reduce(function(ret,param){var parts=param.replace(/\+/g," ").split("="),key=parts[0],val=parts[1];return key=decodeURIComponent(key),val=void 0===val?null:decodeURIComponent(val),ret.hasOwnProperty(key)?Array.isArray(ret[key])?ret[key].push(val):ret[key]=[ret[key],val]:ret[key]=val,ret},{}):styleObject}var factory={};return factory.headerHelper=headerHelper,factory.getStyle=getStyle,factory.parseStyleToObject=parseStyleToObject,factory}function FoundationMQ(foundationApi){function getMediaQueries(){return foundationApi.getSettings().mediaQueries}function match(scenarios){var count=scenarios.length,queries=service.getMediaQueries(),matches=[];if(count>0)for(;count--;){var mq,rule=scenarios[count].media;mq=matchMedia(queries[rule]?queries[rule]:rule),mq.matches&&matches.push({ind:count})}return matches}function collectScenariosFromElement(parentElement){var scenarios=[],templates=[],elements=parentElement.children(),i=0;return angular.forEach(elements,function(el){var elem=angular.element(el);elem.attr("src")&&elem.attr("src").match(/.html$/)?scenarios[i]={media:elem.attr("media"),src:elem.attr("src")}:(templates[i]=elem,scenarios[i]={media:elem.attr("media"),templ:i}),i++}),{scenarios:scenarios,templates:templates}}var service=[];return service.getMediaQueries=getMediaQueries,service.match=match,service.collectScenariosFromElement=collectScenariosFromElement,service}angular.module("foundation.mediaquery",["foundation.core"]).run(mqInitRun).factory("FoundationMQInit",FoundationMQInit).factory("mqHelpers",mqHelpers).service("FoundationMQ",FoundationMQ),mqInitRun.$inject=["FoundationMQInit"],FoundationMQInit.$inject=["mqHelpers","FoundationApi","Utils"],FoundationMQ.$inject=["FoundationApi"]}();
...
"use strict";!function(){var svgDirectives={};angular.forEach(["clipPath","colorProfile","src","cursor","fill","filter","marker","markerStart","markerMid","markerEnd","mask","stroke"],function(attr){svgDirectives[attr]=["$rootScope","$location","$interpolate","$sniffer","urlResolve","computeSVGAttrValue","svgAttrExpressions",function($rootScope,$location,$interpolate,$sniffer,urlResolve,computeSVGAttrValue,svgAttrExpressions){return{restrict:"A",link:function(scope,element,attrs){function updateValue(){var newVal=computeSVGAttrValue(initialUrl);newVal&&attrs[attr]!==newVal&&attrs.$set(attr,newVal)}var initialUrl;svgAttrExpressions.SVG_ELEMENT.test(element[0]&&element[0].toString())&&$location.$$html5&&$sniffer.history&&(initialUrl=attrs[attr],attrs.$observe(attr,updateValue),$rootScope.$on("$locationChangeSuccess",updateValue))}}}]}),angular.module("ngSVGAttributes",[]).factory("urlResolve",[function(){var urlParsingNode=document.createElement("a");return function(url){return urlParsingNode.setAttribute("href",url),urlParsingNode}}]).value("svgAttrExpressions",{FUNC_URI:/^url\((.*)\)$/,SVG_ELEMENT:/SVG[a-zA-Z]*Element/,HASH_PART:/#.*/}).factory("computeSVGAttrValue",["$location","$sniffer","svgAttrExpressions","urlResolve",function($location,$sniffer,svgAttrExpressions,urlResolve){return function(url){var match,fullUrl;return(match=svgAttrExpressions.FUNC_URI.exec(url))&&(fullUrl=0===match[1].indexOf("#")?$location.absUrl().replace(svgAttrExpressions.HASH_PART,"")+match[1]:urlResolve(match[1])),fullUrl?"url("+fullUrl+")":null}}]).directive(svgDirectives)}();
!function(){"use strict";function zfAccordionController($scope){var controller=this,sections=controller.sections=$scope.sections=[],collapsible=(controller.multiOpen=$scope.multiOpen=$scope.multiOpen||!1,controller.collapsible=$scope.collapsible=$scope.multiOpen||$scope.collapsible||!0),autoOpen=controller.autoOpen=$scope.autoOpen=$scope.autoOpen||!0;controller.select=function(selectSection){sections.forEach(function(section){controller.multiOpen?section.scope===selectSection&&(section.scope.active=!section.scope.active):section.scope.active=section.scope===selectSection?collapsible?!section.scope.active:!0:!1})},controller.addSection=function(sectionScope){sections.push({scope:sectionScope}),1===sections.length&&autoOpen===!0&&(sections[0].active=!0,sections[0].scope.active=!0)},controller.closeAll=function(){sections.forEach(function(section){section.scope.active=!1})}}function zfAccordion(){function link(scope,element,attrs,controller){scope.multiOpen=controller.multiOpen="true"===scope.multiOpen?!0:!1,scope.collapsible=controller.collapsible="true"===scope.collapsible?!0:!1,scope.autoOpen=controller.autoOpen="true"===scope.autoOpen?!0:!1}var directive={restrict:"EA",transclude:"true",replace:!0,templateUrl:"components/accordion/accordion.html",controller:"ZfAccordionController",scope:{multiOpen:"@?",collapsible:"@?",autoOpen:"@?"},link:link};return directive}function zfAccordionItem(){function link(scope,element,attrs,controller){scope.active=!1,controller.addSection(scope),scope.activate=function(){controller.select(scope)}}var directive={restrict:"EA",templateUrl:"components/accordion/accordion-item.html",transclude:!0,scope:{title:"@"},require:"^zfAccordion",replace:!0,controller:function(){},link:link};return directive}angular.module("foundation.accordion",[]).controller("ZfAccordionController",zfAccordionController).directive("zfAccordion",zfAccordion).directive("zfAccordionItem",zfAccordionItem),zfAccordionController.$inject=["$scope"]}();
!function(){"use strict";function FoundationActionSheet(foundationApi){function activate(target){foundationApi.publish(target,"show")}function deactivate(target){foundationApi.publish(target,"hide")}var service={};return service.activate=activate,service.deactivate=deactivate,service}function zfActionSheetController($scope){function listenerLogic(e){var el=e.target,insideActionSheet=!1;do if(el.classList&&el.classList.contains("action-sheet-container")){insideActionSheet=!0;break}while(el=el.parentNode);insideActionSheet||hide()}function hide(){content.hide(),container.hide(),content.$apply(),container.$apply()}function toggle(){content.toggle(),container.toggle(),content.$apply(),container.$apply()}{var controller=this,content=controller.content=$scope.content,container=controller.container=$scope.container;angular.element(document.body)}controller.registerContent=function(scope){content=scope,content.active=!1},controller.registerContainer=function(scope){container=scope,container.active=!1},controller.toggle=toggle,controller.hide=hide,controller.registerListener=function(){document.body.addEventListener("click",listenerLogic)},controller.deregisterListener=function(){document.body.removeEventListener("click",listenerLogic)}}function zfActionSheet(foundationApi){function compile(){function preLink(scope,iElement,iAttrs){iAttrs.$set("zf-closable","actionsheet")}function postLink(scope,element,attrs,controller){var id=attrs.id||foundationApi.generateUuid();attrs.$set("id",id),scope.active=!1,foundationApi.subscribe(id,function(msg){"toggle"===msg&&controller.toggle(),("hide"===msg||"close"===msg)&&controller.hide()}),controller.registerContainer(scope),scope.toggle=function(){scope.active=!scope.active},scope.hide=function(){scope.active=!1}}return{pre:preLink,post:postLink}}var directive={restrict:"EA",transclude:!0,replace:!0,templateUrl:"components/actionsheet/actionsheet.html",controller:"ZfActionSheetController",compile:compile};return directive}function zfAsContent(){function link(scope,element,attrs,controller){scope.active=!1,scope.position=scope.position||"bottom",controller.registerContent(scope),scope.toggle=function(){scope.active=!scope.active,scope.active?controller.registerListener():controller.deregisterListener()},scope.hide=function(){scope.active=!1,controller.deregisterListener()}}var directive={restrict:"EA",transclude:!0,replace:!0,templateUrl:"components/actionsheet/actionsheet-content.html",require:"^zfActionSheet",scope:{position:"@?"},link:link};return directive}function zfAsButton(){function link(scope,element,attrs,controller){element.on("click",function(e){controller.toggle(),e.preventDefault()})}var directive={restrict:"EA",transclude:!0,replace:!0,templateUrl:"components/actionsheet/actionsheet-button.html",require:"^zfActionSheet",scope:{title:"@?"},link:link};return directive}angular.module("foundation.actionsheet",["foundation.core"]).controller("ZfActionSheetController",zfActionSheetController).directive("zfActionSheet",zfActionSheet).directive("zfAsContent",zfAsContent).directive("zfAsButton",zfAsButton).service("FoundationActionSheet",FoundationActionSheet),FoundationActionSheet.$inject=["FoundationApi"],zfActionSheetController.$inject=["$scope","FoundationApi"],zfActionSheet.$inject=["FoundationApi"],zfAsContent.$inject=["FoundationApi"],zfAsButton.$inject=["FoundationApi"]}();
!function(){"use strict";function zfClose(foundationApi){function link(scope,element,attrs){var targetId="";if(attrs.zfClose)targetId=attrs.zfClose;else{for(var parentElement=!1,tempElement=element.parent();parentElement===!1;)"BODY"==tempElement[0].nodeName&&(parentElement=""),"undefined"!=typeof tempElement.attr("zf-closable")&&tempElement.attr("zf-closable")!==!1&&(parentElement=tempElement),tempElement=tempElement.parent();targetId=parentElement.attr("id")}element.on("click",function(e){foundationApi.publish(targetId,"close"),e.preventDefault()})}var directive={restrict:"A",link:link};return directive}function zfOpen(foundationApi){function link(scope,element,attrs){element.on("click",function(e){foundationApi.publish(attrs.zfOpen,"open"),e.preventDefault()})}var directive={restrict:"A",link:link};return directive}function zfToggle(foundationApi){function link(scope,element,attrs){element.on("click",function(e){foundationApi.publish(attrs.zfToggle,"toggle"),e.preventDefault()})}var directive={restrict:"A",link:link};return directive}function zfEscClose(foundationApi){function link(scope,element){element.on("keyup",function(e){27===e.keyCode&&foundationApi.closeActiveElements(),e.preventDefault()})}var directive={restrict:"A",link:link};return directive}function zfSwipeClose(foundationApi){function link($scope,element,attrs){var swipeDirection,hammerElem;switch(Hammer&&(hammerElem=new Hammer(element[0]),hammerElem.get("swipe").set({direction:Hammer.DIRECTION_ALL,threshold:5,velocity:.5})),attrs.zfSwipeClose){case"right":swipeDirection="swiperight";break;case"left":swipeDirection="swipeleft";break;case"up":swipeDirection="swipeup";break;case"down":swipeDirection="swipedown";break;default:swipeDirection="swipe"}hammerElem.on(swipeDirection,function(){foundationApi.publish(attrs.id,"close")})}var directive={restrict:"A",link:link};return directive}function zfHardToggle(foundationApi){function link(scope,element,attrs){element.on("click",function(e){foundationApi.closeActiveElements({exclude:attrs.zfHardToggle}),foundationApi.publish(attrs.zfHardToggle,"toggle"),e.preventDefault()})}var directive={restrict:"A",link:link};return directive}angular.module("foundation.common",["foundation.core"]).directive("zfClose",zfClose).directive("zfOpen",zfOpen).directive("zfToggle",zfToggle).directive("zfEscClose",zfEscClose).directive("zfSwipeClose",zfSwipeClose).directive("zfHardToggle",zfHardToggle),zfClose.$inject=["FoundationApi"],zfOpen.$inject=["FoundationApi"],zfToggle.$inject=["FoundationApi"],zfEscClose.$inject=["FoundationApi"],zfSwipeClose.$inject=["FoundationApi"],zfHardToggle.$inject=["FoundationApi"]}();
!function(){"use strict";function iconic(){function getAccess(){return iconicObject}var iconicObject=IconicJS(),service={};return service.getAccess=getAccess,service}function zfIconic(iconic,foundationApi,$compile){function compile(){function preLink(scope,element,attrs){if(scope.iconDir?(assetPath=scope.iconDir,"/"!==assetPath.charAt(assetPath.length-1)&&(assetPath+="/")):assetPath="assets/img/iconic/",scope.dynSrc?attrs.$set("data-src",scope.dynSrc):scope.dynIcon?attrs.$set("data-src",assetPath+scope.dynIcon+".svg"):scope.icon?attrs.$set("data-src",assetPath+scope.icon+".svg"):attrs.$set("data-src",attrs.src),!element.hasClass("iconic-sm")&&!element.hasClass("iconic-md")&&!element.hasClass("iconic-lg")){var iconicClass;switch(scope.size){case"small":iconicClass="iconic-sm";break;case"medium":iconicClass="iconic-md";break;case"large":iconicClass="iconic-lg";break;default:iconicClass="iconic-fluid"}element.addClass(iconicClass)}contents=element[0].outerHTML}function postLink(scope,element){function reinjectSvg(newSrc){svgElement&&(svgElement.empty(),svgElement.append(angular.element(contents)),svgElement.attr("data-src",newSrc),injectSvg(svgElement[0]))}function injectSvg(element){ico.inject(element,{each:function(injectedElem){var angElem=angular.element(injectedElem);svgElement=$compile(angElem)(angElem.scope())}})}var svgElement,ico=iconic.getAccess();injectSvg(element[0]),foundationApi.subscribe("resize",function(){ico.update(element[0])}),scope.dynSrc&&scope.$watch("dynSrc",function(newVal,oldVal){newVal&&newVal!=oldVal&&reinjectSvg(scope.dynSrc)}),scope.dynIcon&&scope.$watch("dynIcon",function(newVal,oldVal){newVal&&newVal!=oldVal&&reinjectSvg(assetPath+scope.dynIcon+".svg")})}var contents,assetPath;return{pre:preLink,post:postLink}}var directive={restrict:"A",template:"<img ng-transclude>",transclude:!0,replace:!0,scope:{dynSrc:"=?",dynIcon:"=?",size:"@?",icon:"@",iconDir:"@?"},compile:compile};return directive}angular.module("foundation.iconic",[]).service("Iconic",iconic).directive("zfIconic",zfIconic),zfIconic.$inject=["Iconic","FoundationApi","$compile","$location"]}();
!function(){"use strict";function zfInterchange($compile,$http,$templateCache,foundationApi,foundationMQ){function link(scope,element,attrs,ctrl,transclude){function templateLoader(templateUrl){return $http.get(templateUrl,{cache:$templateCache})}function collectInformation(el){var data=foundationMQ.collectScenariosFromElement(el);scenarios=data.scenarios,innerTemplates=data.templates}function checkScenario(scenario){return!current||current!==scenario}{var childScope,current,scenarios,innerTemplates;foundationMQ.getMediaQueries()}foundationApi.subscribe("resize",function(){transclude(function(clone,newScope){scenarios&&innerTemplates||collectInformation(clone);var ruleMatches=foundationMQ.match(scenarios),scenario=0===ruleMatches.length?null:scenarios[ruleMatches[0].ind];if(scenario&&checkScenario(scenario)){if(childScope&&(childScope.$destroy(),childScope=null),"undefined"!=typeof scenario.templ){childScope=newScope;var tmp=document.createElement("div");tmp.appendChild(innerTemplates[scenario.templ][0]),element.html(tmp.innerHTML),$compile(element.contents())(childScope),current=scenario}else{var loader=templateLoader(scenario.src);loader.success(function(html){childScope=newScope,element.html(html)}).then(function(){$compile(element.contents())(childScope),current=scenario})}}})}),foundationApi.publish("resize","initial resize")}var directive={restrict:"EA",transclude:"element",scope:{position:"@"},replace:!0,template:"<div></div>",link:link};return directive}angular.module("foundation.interchange",["foundation.core","foundation.mediaquery"]).directive("zfInterchange",zfInterchange),zfInterchange.$inject=["$compile","$http","$templateCache","FoundationApi","FoundationMQ"]}();
!function(){"use strict";function FoundationModal(foundationApi,ModalFactory){function activate(target){foundationApi.publish(target,"show")}function deactivate(target){foundationApi.publish(target,"hide")}function newModal(config){return new ModalFactory(config)}var service={};return service.activate=activate,service.deactivate=deactivate,service.newModal=newModal,service}function modalDirective(foundationApi){function compile(){function preLink(scope,iElement,iAttrs){iAttrs.$set("zf-closable",type)}function postLink(scope,element,attrs){function animate(){scope.overlay||element.css("background","transparent"),foundationApi.animate(element,scope.active,overlayIn,overlayOut),foundationApi.animate(dialog,scope.active,animationIn,animationOut)}function init(){scope.active&&scope.show()}var dialog=angular.element(element.children()[0]);scope.active=scope.active||!1,scope.overlay="false"===attrs.overlay?!1:!0,scope.overlayClose="false"===attrs.overlayClose?!1:!0;var animationIn=attrs.animationIn||"fadeIn",animationOut=attrs.animationOut||"fadeOut",overlayIn="fadeIn",overlayOut="fadeOut";scope.hideOverlay=function(){scope.overlayClose&&scope.hide()},scope.hide=function(){scope.active=!1,animate()},scope.show=function(){scope.active=!0,animate(),dialog.tabIndex=-1,dialog[0].focus()},scope.toggle=function(){scope.active=!scope.active,animate()},init(),foundationApi.subscribe(attrs.id,function(msg){"show"===msg||"open"===msg?scope.show():"close"===msg||"hide"===msg?scope.hide():"toggle"===msg&&scope.toggle(),scope.$root.$$phase||scope.$apply()})}var type="modal";return{pre:preLink,post:postLink}}var directive={restrict:"EA",templateUrl:"components/modal/modal.html",transclude:!0,scope:!0,replace:!0,compile:compile};return directive}function ModalFactory($http,$templateCache,$rootScope,$compile,$timeout,foundationApi){function modalFactory(config){function checkStatus(){if(destroyed)throw"Error: Modal was destroyed. Delete the object and create a new ModalFactory instance."}function activate(){checkStatus(),$timeout(function(){init(!0),foundationApi.publish(id,"show")},0,!1)}function deactivate(){checkStatus(),$timeout(function(){init(!1),foundationApi.publish(id,"hide")},0,!1)}function toggle(){checkStatus(),$timeout(function(){init(!0),foundationApi.publish(id,"toggle")},0,!1)}function init(state){if(!attached&&html.length>0){{container.append(element)}scope.active=state,$compile(element)(scope),attached=!0}}function assembleDirective(){html='<zf-modal id="'+id+'">'+html+"</zf-modal>",element=angular.element(html),scope=$rootScope.$new();for(var prop in props)config[prop]&&element.attr(prop,config[prop])}function destroy(){self.deactivate(),setTimeout(function(){scope.$destroy(),element.remove(),destroyed=!0},3e3)}var html,element,scope,self=this,container=angular.element(config.container||document.body),id=config.id||foundationApi.generateUuid(),attached=!1,destroyed=!1,props=["animationIn","animationOut","overlay","overlayClose"];return config.templateUrl?$http.get(config.templateUrl,{cache:$templateCache}).then(function(response){html=response.data,assembleDirective()}):config.template&&(html=config.template,assembleDirective()),self.activate=activate,self.deactivate=deactivate,self.toggle=toggle,self.destroy=destroy,{activate:activate,deactivate:deactivate,toggle:toggle,destroy:destroy}}return modalFactory}angular.module("foundation.modal",["foundation.core"]).directive("zfModal",modalDirective).factory("ModalFactory",ModalFactory),FoundationModal.$inject=["FoundationApi","ModalFactory"],modalDirective.$inject=["FoundationApi"],ModalFactory.$inject=["$http","$templateCache","$rootScope","$compile","$timeout","FoundationApi"]}();
!function(){"use strict";function FoundationNotification(foundationApi,NotificationFactory){function activate(target){foundationApi.publish(target,"show")}function deactivate(target){foundationApi.publish(target,"hide")}var service={};return service.activate=activate,service.deactivate=deactivate,service}function ZfNotificationController($scope,foundationApi){var controller=this;controller.notifications=$scope.notifications=$scope.notifications||[],controller.addNotification=function(info){var id=foundationApi.generateUuid();info.id=id,$scope.notifications.push(info)},controller.removeNotification=function(id){$scope.notifications.forEach(function(notification){if(notification.id===id){var ind=$scope.notifications.indexOf(notification);$scope.notifications.splice(ind,1)}})},controller.clearAll=function(){for(;$scope.notifications.length>0;)$scope.notifications.pop()}}function zfNotificationSet(foundationApi){function link(scope,element,attrs,controller){scope.position=scope.position?scope.position.split(" ").join("-"):"top-right",foundationApi.subscribe(attrs.id,function(msg){"clearall"===msg?controller.clearAll():(controller.addNotification(msg),scope.$root.$$phase||scope.$apply())})}var directive={restrict:"EA",templateUrl:"components/notification/notification-set.html",controller:"ZfNotificationController",replace:!0,scope:{position:"@"},link:link};return directive}function zfNotification(foundationApi){function compile(){function preLink(scope,iElement,iAttrs){iAttrs.$set("zf-closable","notification")}function postLink(scope,element,attrs,controller){scope.active=!1;var hammerElem,animationIn=attrs.animationIn||"fadeIn",animationOut=attrs.animationOut||"fadeOut";setTimeout(function(){scope.active=!0,foundationApi.animate(element,scope.active,animationIn,animationOut)},50),scope.hide=function(){scope.active=!1,foundationApi.animate(element,scope.active,animationIn,animationOut),setTimeout(function(){controller.removeNotification(scope.notifId)},50)},scope.autoclose&&scope.active&&setTimeout(function(){scope.hide()},parseInt(scope.autoclose)),Hammer&&(hammerElem=new Hammer(element[0]),hammerElem.get("swipe").set({direction:Hammer.DIRECTION_ALL,threshold:5,velocity:.5})),hammerElem.on("swipe",function(){scope.active&&scope.hide()})}return{pre:preLink,post:postLink}}var directive={restrict:"EA",templateUrl:"components/notification/notification.html",replace:!0,transclude:!0,require:"^zfNotificationSet",controller:function(){},scope:{title:"=?",content:"=?",image:"=?",notifId:"=",color:"=?",autoclose:"=?"},compile:compile};return directive}function zfNotificationStatic(foundationApi){function compile(){function preLink(scope,iElement,iAttrs){iAttrs.$set("zf-closable",type)}function postLink(scope,element,attrs){scope.position=attrs.position?attrs.position.split(" ").join("-"):"top-right";var animationIn=attrs.animationIn||"fadeIn",animationOut=attrs.animationOut||"fadeOut";foundationApi.subscribe(attrs.id,function(msg){"show"==msg||"open"==msg?(scope.show(),scope.autoclose&&setTimeout(function(){scope.hide()},parseInt(scope.autoclose))):"close"==msg||"hide"==msg?scope.hide():"toggle"==msg&&(scope.toggle(),scope.autoclose&&setTimeout(function(){scope.toggle()},parseInt(scope.autoclose))),foundationApi.animate(element,scope.active,animationIn,animationOut),scope.$apply()}),scope.hide=function(){scope.active=!1,foundationApi.animate(element,scope.active,animationIn,animationOut)},scope.show=function(){scope.active=!0,foundationApi.animate(element,scope.active,animationIn,animationOut)},scope.toggle=function(){scope.active=!scope.active,foundationApi.animate(element,scope.active,animationIn,animationOut)}}var type="notification";return{pre:preLink,post:postLink}}var directive={restrict:"EA",templateUrl:"components/notification/notification-static.html",replace:!0,transclude:!0,scope:{title:"@?",content:"@?",image:"@?",color:"@?",autoclose:"@?"},compile:compile};return directive}function zfNotify(foundationApi){function link(scope,element,attrs){element.on("click",function(e){foundationApi.publish(attrs.zfNotify,{title:scope.title,content:scope.content,color:scope.color,image:scope.image,autoclose:scope.autoclose}),e.preventDefault()})}var directive={restrict:"A",scope:{title:"@?",content:"@?",color:"@?",image:"@?",autoclose:"@?"},link:link};return directive}function NotificationFactory($http,$templateCache,$rootScope,$compile,$timeout,foundationApi){function notificationFactory(config){function checkStatus(){if(destroyed)throw"Error: Notification Set was destroyed. Delete the object and create a new NotificationFactory instance."}function addNotification(notification){checkStatus(),$timeout(function(){init(!0,notification),foundationApi.publish(id,notification)},0,!1)}function clearAll(){checkStatus(),$timeout(function(){foundationApi.publish(id,"clearall")},0,!1)}function init(state,notification){if(!attached&&html.length>0){{container.append(element)}scope.notifications=[notification],$compile(element)(scope),attached=!0}}function assembleDirective(){html='<zf-notification-set id="'+id+'"></zf-notification-set>',element=angular.element(html),scope=$rootScope.$new();for(var prop in props)config[prop]&&element.attr(prop,config[prop])}function destroy(){self.clearAll(),setTimeout(function(){scope.$destroy(),element.remove(),destroyed=!0},3e3)}var html,element,scope,self=this,container=angular.element(config.container||document.body),id=config.id||foundationApi.generateUuid(),attached=!1,destroyed=!1,props=["position"];return assembleDirective(),self.addNotification=addNotification,self.clearAll=clearAll,self.destroy=destroy,{addNotification:addNotification,clearAll:clearAll,destroy:destroy}}return notificationFactory}angular.module("foundation.notification",["foundation.core"]).controller("ZfNotificationController",ZfNotificationController).directive("zfNotificationSet",zfNotificationSet).directive("zfNotification",zfNotification).directive("zfNotificationStatic",zfNotificationStatic).directive("zfNotify",zfNotify).factory("NotificationFactory",NotificationFactory).service("FoundationNotification",FoundationNotification),FoundationNotification.$inject=["FoundationApi","NotificationFactory"],ZfNotificationController.$inject=["$scope","FoundationApi"],zfNotificationSet.$inject=["FoundationApi"],zfNotification.$inject=["FoundationApi"],zfNotificationStatic.$inject=["FoundationApi"],zfNotify.$inject=["FoundationApi"],NotificationFactory.$inject=["$http","$templateCache","$rootScope","$compile","$timeout","FoundationApi"]}();
!function(){"use strict";function FoundationOffcanvas(foundationApi){function activate(target){foundationApi.publish(target,"show")}function deactivate(target){foundationApi.publish(target,"hide")}var service={};return service.activate=activate,service.deactivate=deactivate,service}function zfOffcanvas(foundationApi){function compile(){function preLink(scope,iElement,iAttrs){iAttrs.$set("zf-closable",type),document.body.classList.add("has-off-canvas")}function postLink(scope,element,attrs){scope.position=scope.position||"left",scope.active=!1,foundationApi.subscribe(attrs.id,function(msg){"show"===msg||"open"===msg?scope.show():"close"===msg||"hide"===msg?scope.hide():"toggle"===msg&&scope.toggle(),scope.$apply()}),scope.hide=function(){scope.active=!1},scope.show=function(){scope.active=!0},scope.toggle=function(){scope.active=!scope.active}}var type="offcanvas";return{pre:preLink,post:postLink}}var directive={restrict:"EA",templateUrl:"components/offcanvas/offcanvas.html",transclude:!0,scope:{position:"@"},replace:!0,compile:compile};return directive}angular.module("foundation.offcanvas",["foundation.core"]).directive("zfOffcanvas",zfOffcanvas).service("FoundationOffcanvas",FoundationOffcanvas),FoundationOffcanvas.$inject=["FoundationApi"],zfOffcanvas.$inject=["FoundationApi"]}();
!function(){"use strict";function FoundationPanel(foundationApi){function activate(target){foundationApi.publish(target,"show")}function deactivate(target){foundationApi.publish(target,"hide")}var service={};return service.activate=activate,service.deactivate=deactivate,service}function zfPanel(foundationApi){function compile(){function preLink(scope,iElement,iAttrs){iAttrs.$set("zf-closable",type),scope.position=scope.position||"left",scope.positionClass="panel-"+scope.position}function postLink(scope,element,attrs){scope.active=!1;var animationIn,animationOut,globalQueries=foundationApi.getSettings().mediaQueries;"left"===scope.position?(animationIn=attrs.animationIn||"slideInRight",animationOut=attrs.animationOut||"slideOutLeft"):"right"===scope.position?(animationIn=attrs.animationIn||"slideInLeft",animationOut=attrs.animationOut||"slideOutRight"):"top"===scope.position?(animationIn=attrs.animationIn||"slideInDown",animationOut=attrs.animationOut||"slideOutUp"):"bottom"===scope.position&&(animationIn=attrs.animationIn||"slideInUp",animationOut=attrs.animationOut||"slideOutBottom"),foundationApi.subscribe(attrs.id,function(msg){"show"==msg||"open"==msg?scope.show():"close"==msg||"hide"==msg?scope.hide():"toggle"==msg&&scope.toggle(),scope.$apply()}),scope.hide=function(){scope.active&&(scope.active=!1,foundationApi.animate(element,scope.active,animationIn,animationOut))},scope.show=function(){scope.active||(scope.active=!0,foundationApi.animate(element,scope.active,animationIn,animationOut))},scope.toggle=function(){scope.active=!scope.active,foundationApi.animate(element,scope.active,animationIn,animationOut)},element.on("click",function(e){var srcEl=e.srcElement;!matchMedia(globalQueries.medium).matches&&srcEl.href&&srcEl.href.length>0&&(scope.hide(),foundationApi.animate(element,scope.active,animationIn,animationOut))})}var type="panel";return{pre:preLink,post:postLink}}var directive={restrict:"EA",templateUrl:"components/panel/panel.html",transclude:!0,scope:{position:"@?"},replace:!0,compile:compile};return directive}angular.module("foundation.panel",["foundation.core"]).directive("zfPanel",zfPanel).service("FoundationPanel",FoundationPanel),FoundationPanel.$inject=["FoundationApi"],zfPanel.$inject=["FoundationApi"]}();
!function(){"use strict";function FoundationPopup(foundationApi){function activate(target){foundationApi.publish(target,"show")}function deactivate(target){foundationApi.publish(target,"hide")}var service={};return service.activate=activate,service.deactivate=deactivate,service}function zfPopup(foundationApi){function compile(){function preLink(scope,iElement,iAttrs){iAttrs.$set("zf-closable","popup")}function postLink(scope,element,attrs){function tetherElement(target){tetherInit||(scope.target=document.getElementById(scope.target?scope.target:target),tether=new Tether({element:element[0],target:scope.target,attachment:attachment,targetAttachment:targetAttachment,enable:!1}),tetherInit=!0)}scope.active=!1,scope.target=scope.target||!1;var attachment=scope.pinTo||"top center",targetAttachment=scope.pinAt||"bottom center",tetherInit=!1,tether={};foundationApi.subscribe(attrs.id,function(msg){"show"===msg[0]||"open"===msg[0]?scope.show(msg[1]):"close"===msg[0]||"hide"===msg[0]?scope.hide():"toggle"===msg[0]&&scope.toggle(msg[1]),scope.$apply()}),scope.hide=function(){scope.active=!1,tetherElement(),tether.disable()},scope.show=function(newTarget){scope.active=!0,tetherElement(newTarget),tether.enable()},scope.toggle=function(newTarget){scope.active=!scope.active,tetherElement(newTarget),scope.active?tether.enable():tether.disable()}}return{pre:preLink,post:postLink}}var directive={restrict:"EA",transclude:!0,replace:!0,templateUrl:"components/popup/popup.html",scope:{pinTo:"@?",pinAt:"@?",target:"@?"},compile:compile};return directive}function zfPopupToggle(foundationApi){function link(scope,element,attrs){var target=attrs.zfPopupToggle,id=attrs.id||foundationApi.generateUuid();attrs.$set("id",id),element.on("click",function(e){foundationApi.publish(target,["toggle",id]),e.preventDefault()})}var directive={restrict:"A",link:link};return directive}angular.module("foundation.popup",["foundation.core"]).directive("zfPopup",zfPopup).directive("zfPopupToggle",zfPopupToggle).service("FoundationPopup",FoundationPopup),FoundationPopup.$inject=["FoundationApi"],zfPopup.$inject=["FoundationApi"],zfPopupToggle.$inject=["FoundationApi"]}();
!function(){"use strict";function FoundationTabs(foundationApi){function activate(target){foundationApi.publish(target,"show")}var service={};return service.activate=activate,service}function ZfTabsController($scope,foundationApi){var controller=this,tabs=controller.tabs=$scope.tabs=[],id="";controller.select=function(selectTab){tabs.forEach(function(tab){tab.active=!1,tab.scope.active=!1,tab.scope===selectTab&&(foundationApi.publish(id,["activate",tab]),tab.active=!0,tab.scope.active=!0)})},controller.addTab=function(tabScope){tabs.push({scope:tabScope,active:!1,parentContent:controller.id}),1===tabs.length&&(tabs[0].active=!0,tabScope.active=!0)},controller.getId=function(){return id},controller.setId=function(newId){id=newId}}function zfTabs(foundationApi){function link(scope,element,attrs,controller){scope.id=attrs.id||foundationApi.generateUuid(),scope.showTabContent="true"!==scope.displaced,attrs.$set("id",scope.id),controller.setId(scope.id);var updateTabs=function(){foundationApi.publish(scope.id+"-tabs",scope.tabs)};foundationApi.subscribe(scope.id+"-get-tabs",function(){updateTabs()})}var directive={restrict:"EA",transclude:"true",replace:!0,templateUrl:"components/tabs/tabs.html",controller:"ZfTabsController",scope:{displaced:"@?"},link:link};return directive}function zfTabContent(foundationApi){function link(scope){scope.tabs=scope.tabs||[];var id=scope.target;foundationApi.subscribe(id,function(msg){if("activate"===msg[0]){{msg[1]}scope.tabs.forEach(function(tab){tab.scope.active=!1,tab.active=!1,tab.scope.id===id&&(tab.scope.active=!0,tab.active=!0)})}}),0===scope.tabs.length&&(foundationApi.subscribe(id+"-tabs",function(tabs){scope.tabs=tabs}),foundationApi.publish(id+"-get-tabs",""))}var directive={restrict:"A",transclude:"true",replace:!0,scope:{tabs:"=?",target:"@"},templateUrl:"components/tabs/tab-content.html",link:link};return directive}function zfTab(foundationApi){function link(scope,element,attrs,controller,transclude){scope.id=attrs.id||foundationApi.generateUuid(),scope.active=!1,scope.transcludeFn=transclude,controller.addTab(scope),foundationApi.subscribe(scope.id,function(msg){("show"===msg||"open"===msg||"activate"===msg)&&scope.makeActive()}),scope.makeActive=function(){controller.select(scope)}}var directive={restrict:"EA",templateUrl:"components/tabs/tab.html",transclude:!0,scope:{title:"@"},require:"^zfTabs",replace:!0,link:link};return directive}function zfTabIndividual(foundationApi){function link(scope,element,attrs){{var tab=scope.$eval(attrs.tab);tab.scope.id}tab.scope.transcludeFn(tab.scope,function(tabContent){element.append(tabContent)}),foundationApi.subscribe(tab.scope.id,function(){foundationApi.publish(tab.parentContent,["activate",tab.scope.id]),scope.$apply()})}var directive={restrict:"EA",transclude:"true",link:link};return directive}function zfTabHref(foundationApi){function link(scope,element,attrs){function makeActive(){element.parent().children().removeClass("is-active"),element.addClass("is-active")}var target=attrs.zfTabHref;foundationApi.subscribe(target,function(msg){("activate"===msg||"show"===msg||"open"===msg)&&makeActive()}),element.on("click",function(e){foundationApi.publish(target,"activate"),makeActive(),e.preventDefault()})}var directive={restrict:"A",replace:!1,link:link};return directive}function zfTabCustom(){function link(scope,element){var children=element.children();angular.element(children[0]).addClass("is-active")}var directive={restrict:"A",replace:!1,link:link};return directive}function zfTabContentCustom(foundationApi){function link(scope,element){function activateTabs(tabId){var tabNodes=element.children();angular.forEach(tabNodes,function(node){var el=angular.element(node);el.removeClass("is-active"),el.attr("id")===tabId&&el.addClass("is-active")})}var tabs=[],children=element.children();angular.forEach(children,function(node){if(node.id){var tabId=node.id;if(tabs.push(tabId),foundationApi.subscribe(tabId,function(msg){("activate"===msg||"show"===msg||"open"===msg)&&activateTabs(tabId)}),1===tabs.length){var el=angular.element(node);el.addClass("is-active")}}})}return{restrict:"A",link:link}}angular.module("foundation.tabs",["foundation.core"]).controller("ZfTabsController",ZfTabsController).directive("zfTabs",zfTabs).directive("zfTabContent",zfTabContent).directive("zfTab",zfTab).directive("zfTabIndividual",zfTabIndividual).directive("zfTabHref",zfTabHref).directive("zfTabCustom",zfTabCustom).directive("zfTabContentCustom",zfTabContentCustom).service("FoundationTabs",FoundationTabs),FoundationTabs.$inject=["FoundationApi"],ZfTabsController.$inject=["$scope","FoundationApi"],zfTabs.$inject=["FoundationApi"],zfTabContent.$inject=["FoundationApi"],zfTab.$inject=["FoundationApi"],zfTabIndividual.$inject=["FoundationApi"],zfTabHref.$inject=["FoundationApi"],zfTabCustom.$inject=["FoundationApi"],zfTabContentCustom.$inject=["FoundationApi"]}();
